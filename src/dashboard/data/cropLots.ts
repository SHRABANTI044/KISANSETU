import tomatoImg from "../../assets/images/lots/tomato.jpg";
import tomatoPlantsImg from "../../assets/images/lots/tomato-plants.jpg";
import onionImg from "../../assets/images/lots/onion.jpg";
import potatoImg from "../../assets/images/lots/potato.jpg";
import chilliImg from "../../assets/images/lots/green-chilli.jpg";
import cauliflowerImg from "../../assets/images/lots/cauliflower.jpg";
import crateImg from "../../assets/images/mandi-market.jpg";

/**
 * FRONTEND MOCK DATA for the My Lots page.
 * Typed structures kept API-ready — swap these for server data later
 * without touching the UI.
 */

export type LotStatus = "active" | "sold" | "draft" | "expired";

export interface CropLot {
  id: string;
  crop: string;
  cropKey: string;
  quantity: number;
  unit: "kg" | "quintal" | "tonne";
  grade: string;
  status: LotStatus;
  location: string;
  harvestDate: string;
  harvestDateIso: string;
  expectedPrice?: number;
  soldPrice?: number;
  buyer?: string;
  transactionCompleted?: string;
  offers: number;
  views: number;
  postedOn: string;
  postedOnIso: string;
  lastSaved?: string;
  lastUpdated: string;
  description: string;
  image: string;
  images: string[];
}

export interface LotOffer {
  id: string;
  buyer: string;
  location: string;
  price: number;
  quantity: number;
  unit: string;
  status: "pending" | "accepted" | "rejected";
}

export interface LotViewer {
  id: string;
  name: string;
  business: string;
  location: string;
  lastViewed: string;
  initials: string;
}

const DEFAULT_LOCATION = "Kothur, Ahmednagar";

export const INITIAL_LOTS: CropLot[] = [
  {
    id: "LOT250525001",
    crop: "Tomato (Hybrid)",
    cropKey: "Tomato",
    quantity: 500,
    unit: "kg",
    grade: "Grade A",
    status: "active",
    location: DEFAULT_LOCATION,
    harvestDate: "25 May 2025",
    harvestDateIso: "2025-05-25",
    expectedPrice: 23,
    offers: 3,
    views: 245,
    postedOn: "25 May 2025",
    postedOnIso: "2025-05-25",
    lastUpdated: "26 May 2025, 09:15 AM",
    description: "Freshly harvested, good quality tomatoes. Ready for immediate delivery.",
    image: tomatoImg,
    images: [tomatoImg, crateImg, tomatoPlantsImg],
  },
  {
    id: "LOT250520002",
    crop: "Onion",
    cropKey: "Onion",
    quantity: 300,
    unit: "kg",
    grade: "Grade B",
    status: "active",
    location: DEFAULT_LOCATION,
    harvestDate: "20 May 2025",
    harvestDateIso: "2025-05-20",
    expectedPrice: 18,
    offers: 1,
    views: 120,
    postedOn: "20 May 2025",
    postedOnIso: "2025-05-20",
    lastUpdated: "21 May 2025, 11:40 AM",
    description: "Well-cured Nashik red onions, medium size. Suitable for long storage.",
    image: onionImg,
    images: [onionImg, crateImg],
  },
  {
    id: "LOT250515003",
    crop: "Potato",
    cropKey: "Potato",
    quantity: 1000,
    unit: "kg",
    grade: "Grade A",
    status: "sold",
    location: DEFAULT_LOCATION,
    harvestDate: "15 May 2025",
    harvestDateIso: "2025-05-15",
    soldPrice: 16,
    buyer: "Pune Fresh Mart",
    transactionCompleted: "18 May 2025",
    offers: 4,
    views: 310,
    postedOn: "15 May 2025",
    postedOnIso: "2025-05-15",
    lastUpdated: "18 May 2025, 06:20 PM",
    description: "Clean, medium-size potatoes ideal for retail and processing.",
    image: potatoImg,
    images: [potatoImg, crateImg],
  },
  {
    id: "LOT250528004",
    crop: "Green Chilli",
    cropKey: "Green Chilli",
    quantity: 200,
    unit: "kg",
    grade: "Grade A",
    status: "active",
    location: DEFAULT_LOCATION,
    harvestDate: "28 May 2025",
    harvestDateIso: "2025-05-28",
    expectedPrice: 40,
    offers: 2,
    views: 98,
    postedOn: "28 May 2025",
    postedOnIso: "2025-05-28",
    lastUpdated: "29 May 2025, 08:10 AM",
    description: "Crisp, spicy green chillies harvested this morning.",
    image: chilliImg,
    images: [chilliImg, crateImg],
  },
  {
    id: "LOT250529005",
    crop: "Cauliflower",
    cropKey: "Cauliflower",
    quantity: 400,
    unit: "kg",
    grade: "Grade A",
    status: "draft",
    location: DEFAULT_LOCATION,
    harvestDate: "",
    harvestDateIso: "",
    expectedPrice: 25,
    offers: 0,
    views: 0,
    postedOn: "",
    postedOnIso: "",
    lastSaved: "29 May 2025",
    lastUpdated: "29 May 2025, 07:55 PM",
    description: "Compact white cauliflower heads with healthy leaves.",
    image: cauliflowerImg,
    images: [cauliflowerImg],
  },
];

export const INITIAL_OFFERS: Record<string, LotOffer[]> = {
  LOT250525001: [
    { id: "of1", buyer: "FreshMart", location: "Pune", price: 24, quantity: 500, unit: "kg", status: "pending" },
    { id: "of2", buyer: "Pune Fresh Mart", location: "Pune", price: 23.5, quantity: 300, unit: "kg", status: "pending" },
    { id: "of3", buyer: "Agro Traders", location: "Nashik", price: 22.75, quantity: 500, unit: "kg", status: "pending" },
  ],
  LOT250520002: [
    { id: "of4", buyer: "Lasalgaon Traders", location: "Lasalgaon", price: 17.5, quantity: 300, unit: "kg", status: "pending" },
  ],
  LOT250528004: [
    { id: "of5", buyer: "FreshMart", location: "Pune", price: 38, quantity: 100, unit: "kg", status: "pending" },
    { id: "of6", buyer: "Green Basket", location: "Ahmednagar", price: 39.25, quantity: 200, unit: "kg", status: "pending" },
  ],
  LOT250515003: [
    { id: "of7", buyer: "Pune Fresh Mart", location: "Pune", price: 16, quantity: 1000, unit: "kg", status: "accepted" },
  ],
};

export const LOT_VIEWERS: Record<string, LotViewer[]> = {
  LOT250525001: [
    { id: "v1", name: "Suresh Kale", business: "FreshMart Retail", location: "Pune", lastViewed: "10 min ago", initials: "SK" },
    { id: "v2", name: "Anita Deshmukh", business: "Pune Fresh Mart", location: "Pune", lastViewed: "32 min ago", initials: "AD" },
    { id: "v3", name: "Vikram Shinde", business: "Agro Traders", location: "Nashik", lastViewed: "1 hr ago", initials: "VS" },
    { id: "v4", name: "Meera Joshi", business: "Green Basket", location: "Ahmednagar", lastViewed: "2 hrs ago", initials: "MJ" },
    { id: "v5", name: "Rohit Pawar", business: "Pawar Wholesale", location: "Sangamner", lastViewed: "5 hrs ago", initials: "RP" },
  ],
  LOT250520002: [
    { id: "v6", name: "Nitin Gadakh", business: "Lasalgaon Traders", location: "Lasalgaon", lastViewed: "26 min ago", initials: "NG" },
    { id: "v7", name: "Anita Deshmukh", business: "Pune Fresh Mart", location: "Pune", lastViewed: "3 hrs ago", initials: "AD" },
  ],
  LOT250528004: [
    { id: "v8", name: "Suresh Kale", business: "FreshMart Retail", location: "Pune", lastViewed: "48 min ago", initials: "SK" },
    { id: "v9", name: "Meera Joshi", business: "Green Basket", location: "Ahmednagar", lastViewed: "1 hr ago", initials: "MJ" },
  ],
  LOT250515003: [
    { id: "v10", name: "Vikram Shinde", business: "Agro Traders", location: "Nashik", lastViewed: "2 days ago", initials: "VS" },
  ],
};

export const GRADE_OPTIONS = ["Grade A", "Grade B", "Grade C", "Other"];
export const UNIT_OPTIONS = ["kg", "quintal", "tonne"] as const;

/* ------------------------------ Formatting ------------------------------- */

export function formatPrice(value?: number): string {
  if (value === undefined) return "—";
  return `₹${value.toFixed(2)}`;
}

export function todayLabel(): string {
  return new Date().toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });
}

export function todayIso(): string {
  return new Date().toISOString().slice(0, 10);
}

export function nowTimestamp(): string {
  const d = new Date();
  const date = d.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });
  const time = d.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true });
  return `${date}, ${time}`;
}
